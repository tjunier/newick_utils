...
\startalignment[center]
	% TODO: ConTeXtify the commented-out stuff below
...
\placetable[][tbl:luaedPredefVar]{%
	...
	\starttabulate[|c|l|l|]
		name \NC (Lua) type \NC meaning (refers to the current node)\NC\NR
		\HL
		\code{a} \NC integer \NC number of ancestors  \NC\NR
		\code{b} \NC number \NC support value (or \code{nil}) \NC\NR
		\code{c} \NC integer \NC number of children (direct descendants) \NC\NR
		\code{D} \NC integer \NC total number of descendants (includes children) \NC\NR
		\code{d} \NC number \NC depth (distance to root) \NC\NR
		\code{i} \NC Boolean \NC true iff node is strictly internal (i.e., not root!) \NC\NR
		\code{lbl} \NC string \NC label \NC\NR
		\code{l} (ell) \NC Boolean \NC true iff node is a leaf \NC\NR
		\code{L} \NC number \NC parent edge length \NC\NR
		\code{N} \NC node \NC the current node itself \NC\NR
		\code{r} \NC Boolean \NC true iff node is the root
	\stoptabulate
}
...
\code{l and a <= 3} \NC leaves with 3 ancestors or less \NC\NR
\code{i and (b \~{ }= nil) and (b >= 95)} \NC internal nodes with support $\geq$ 95\% \NC\NR 
\code{i and (b \~{ }= nil) and (b < 50)} \NC unsupported nodes (less than 50\%) \NC\NR
\code{not r} \NC all nodes except the root \NC\NR
...
\placetable[][tbl:luaedPredefFunc]{%
	...
	\starttabulate[|c|l|c|]
		...
		\HL
		\code{o}	\NC splice out the node 									\NC yes \NC\NR
		\code{s}	\NC print the subtree rooted at the node 	\NC no \NC\NR
		\code{u} 	\NC delete ("unlink") the node (and all descendants) \NC yes
	\stoptabulate
}
...
\placetable[][tbl:luaedNodeProp]{%
	...
	%\center
	\starttabulate[llcl]
		name \NC type \NC mode \NC meaning \NC\NR
		\HL
		\code{len}, \code{L} 	\NC number 	\NC rw \NC parent edge's length \NC\NR
		\code{lbl}							\NC string 	\NC rw \NC label \NC\NR
		\code{b}								\NC number	\NC ro \NC support value \NC\NR
		\code{par}							\NC node		\NC ro \NC parent \NC\NR
		\code{first\_child}, \code{fc}  \NC node \NC ro \NC first child \NC\NR
		\code{last\_child}, \code{lc} 	\NC node \NC ro \NC last child \NC\NR
		\code{children\_count}, \code{c} 	\NC integer \NC ro \NC number of children \NC\NR
		\code{kids}							\NC table		\NC ro \NC list of children nodes
	\stoptabulate
}
...
\placetable[][tbl:luaedHooks]{%
	...
	\starttabulate[|l|l|]
		hook name \NC called\ldots \NC\NR
		\HL
		\code{start\_run} 	\NC before processing any tree \NC\NR
		\code{start\_tree}	\NC for each tree, before processing \NC\NR
		\code{node}					\NC for each node \NC\NR
		\code{stop\_tree}		\NC for each tree, after processing \NC\NR
		\code{stop\_run}			\NC after processing all trees
	\stoptabulate
}
...
\HL
\labels{} 						\NC {\tt 'lbl \~{}= ""' 'print(lbl)'}  \NC\NR
\labels{} {\tt -I} 		\NC {\tt 'l and lbl \~{}= ""' 'print(lbl)'} \NC\NR
\topology{}						\NC {\tt true 'N.len = ""'} \NC\NR
\topology{} {\tt -I} 	\NC {\tt true 'N.len = ""; if not l then N.lbl = "" end'}
\stoptabulate
\stopalignment
...
\HL
\code{a} \NC numeric \NC number of ancestors of node	 \NC\NR
\code{b} \NC numeric \NC node's support value (or zero) \NC\NR
\code{c} \NC numeric \NC node's number of children (direct) \NC\NR
\code{D} \NC numeric \NC node's number of descendants (includes children) \NC\NR
\code{d} \NC numeric \NC node's depth (distance to root) \NC\NR
\code{i} \NC Boolean \NC true iff node is strictly internal (i.e., not root!) \NC\NR
\code{l} (ell) \NC Boolean \NC true iff node is a leaf \NC\NR
\code{r} \NC Boolean \NC true iff node is the root
\stoptabulate
...
\NC \code{c > 2} \NC multifurcating nodes
\stoptabulate
\stopalignment
...
code \NC effect \NC modifies tree?\NC\NR
\HL
\code{d} \NC delete the node (and all descendants) \NC yes \NC\NR
\code{l} \NC print the node's label \NC no \NC\NR
\code{o} \NC splice out the node \NC yes \NC\NR
\code{s} \NC print the subtree rooted at the node \NC no \NC\NR
\stoptabulate
\stopalignment
...
\HL
\code{a} \NC integer \NC number of ancestors  \NC\NR
\code{b} \NC rational \NC support value \NC\NR
\code{c} \NC integer \NC number of children (direct descendants) \NC\NR
\code{D} \NC integer \NC total number of descendants (includes children) \NC\NR
\code{d} \NC numeric \NC depth (distance to root) \NC\NR
\code{i} \NC Boolean \NC true iff node is strictly internal (i.e., not root!) \NC\NR
\code{lbl} \NC string \NC label \NC\NR
\code{l} (ell) \NC Boolean \NC true iff node is a leaf \NC\NR
\code{L} \NC rational \NC parent edge length \NC\NR
\code{r} \NC Boolean \NC true iff node is the root
\stoptabulate
\stopalignment
...
\startalignment[center]
\begin{tabular}{|l|c|l|}
Scheme \NC \sched{} short form \NC meaning \NC\NR
\HL
\code{not} 	\NC \code{!} 				\NC logical negation \NC\NR
\code{and} 	\NC \code{\NC\NRNC} 			\NC logical and \\
\code{or}  	\NC \code{|} 				\NC logical or \NC\NR
\code{defined?}	\NC \code{def?}	\NC checks if arg is defined
\stoptabulate
\stopalignment
...
\HL
\code{l} (lowercase ell) \NC all leaves \NC\NR
\code{(\NC\NRNC l (<= a 3))} \NC leaves with 3 ancestors or less \\
\code{(\NC\NRNC i (def? 'b) (>= b 95))} \NC internal nodes with support greater than 95\% \\ 
\code{(\NC\NRNC i (def? 'b) (< b 50)} \NC unsupported nodes (less than 50\%) \\
...
code \NC effect \NC modifies tree?\NC\NR
\HL
\code{L! <len>}	\NC sets the node's parent-edge length to \code{len}	\NC yes \NC\NR
\code{lbl! <lbl>}	\NC sets the node's label to \code{lbl}	\NC yes \NC\NR
\code{o} 				\NC splice out the node 									\NC yes \NC\NR
\code{p <arg>} 	\NC print \code{arg}, then a newline 		\NC no \NC\NR
\code{s} 				\NC print the subtree rooted at the node 	\NC no \NC\NR
\code{u} 				\NC delete ("unlink") the node (and all descendants) \NC yes \NC\NR
\stoptabulate
\stopalignment
