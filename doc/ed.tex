...
\ed{}'s limitations: by embedding a Scheme (see \eg{}
\url{http://www.schemers.org}) interpreter (\gnu{} Guile), its flexibility is,
...
for all its limitations, will stay in the package as it does not depend on
Guile. Finally, I understand that Scheme is not the only solution as an
...
{\it selector}) and action are passed together as a Scheme expression. The
expression \texttt{((\& (def? 'b) (>= 95 b)) (s))} parses as follows:
\begin{itemize}
...
Like \ed{} addresses, \sched{} selectors are boolean Scheme expressions normally
involving node properties which are available as predefined variables.  As in
\ed{}, the variables have short names, to keep expressions short.

\begin{center}
\begin{tabular}{cll}
name & type & meaning \\
\hline
\texttt{a} & integer & number of ancestors of node	 \\
\texttt{b} & rational & node's support value \\
\texttt{c} & integer & node's number of children (direct descendants) \\
\texttt{D} & integer & node's total number of descendants (includes children) \\
\texttt{d} & numeric & node's depth (distance to root) \\
\texttt{i} & boolean & true iff node is strictly internal (i.e., not root!) \\
\texttt{lbl} & string & node's label \\
\texttt{l} (ell) & boolean & true iff node is a leaf \\
\texttt{L} & rational & node's parent edge length \\
\texttt{r} & boolean & true iff node is the root
\end{tabular}
\end{center}

Variables \texttt{b} and \texttt{lbl} are both derived from the label, but
\texttt{b} is interpreted as a number, and is undefined if the conversion to a
number fails, or if the node is a leaf. Edge length and depth (\texttt{L} and
\texttt{d}) are undefined (not zero!) if not specified in the Newick tree, as in
cladograms.
