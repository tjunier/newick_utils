...
You can use option \texttt{-w} to set the number of columns available for
display. By default, \display{} will try to find the width of the terminal and
use this. If the output is not a terminal (but, say, a file or pipe), or if for
some reason the width cannot be found or is not defined, then the program uses
