...
textt{-b} the branch length style. For example, th efollowing tree, which was
...
\noindent{}Specifically, we're going to remove the branch lengths, reduce the vertical spacing, and put the leaf labels in italics:
...
Note that we did not \emph{discard} the branch lengths (we could do this with
\topology), because doing so would reduce the tree to a cladogram.
