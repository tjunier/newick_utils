...
In the following, $T$ will be a rooted tree (which is a tree in which some node is singled out and called the root, which we will note $R$). I will use the following notation to define paths in the tree: $[a, b]$ is the path from $a$ to $b$; $[a, b)$ is the path from $a$ to the predecessor of $b$, $(a, b]$ is the path from the successor of $a$ to $b$. COncatenation will be noted by appending path notations, e.g. a path from $a$ to $c$ passing through $b$ can be written $[a,b) [b,c]$ or $[a,b) [b] (b,c]$.

\begin{dfn}
A path that connects the root to any node of the tree I call a \emph{lineage}. By the definition of a tree (acyclic, connected graph), there is exactly one path between any two nodes, so there is always a lineage to $n$ for any node $n$, and this lineage is unique. So we can unambiguously talk of the lineage of $n$ to mean the path from the $R$ to $n$.
\end{dfn}

\begin{dfn}{Ancestry relation}
Let $n$ be a node of $T$, and $L$ the lineage of $n$. A node $a \neq n$ is called an \emph{ancestor} of $n$ iff $L$ passes though $a$. We note this $a < n$ (mnemonic: the arrow points to the root).
\end{dfn}

\begin{prop}{Comparability of nodes in a lineage}
Let $n$ be a node of $T$, $L$ its lineage, and $a, b$ two nodes of $L$. Then either: i) $a < b$ or ii) $a = b$ or iii) $b < a$. 
\end{prop}

\begin{proof}
Since $L$ is the unique path from $R$ to $n$, and $a$ belongs to it, we can express $L$ as $[R,a) [a] (a,n]$. Now $b$ also belongs to this path.
\end{proof}

In other words, two nodes in a given lineage are comparable: one is the ancestor of the other, or else they are the same. This is \emph{not} true of any two nodes in $T$, of course - in particular, two leaves are never comparable.

\begin{prop}{Antisymmetry of ancestry}
Let $n$ be a node of $T$, $L$ the lineage of $n$, and $a$, $b$ two nodes belonging to $L$, with $a \neq{} b$. Then either $a$ is an ancestor of $b$, or $b$ is an ancestor of $a$. 
\end{prop}

\begin{prop}{Transitivity of ancestry}
Let $n$ be a node of $T$, $L$ the lineage of $n$, and $a, b, c \in L$ such that $a < b$ and $b < c$. Then $a < c$
\end{prop}


\begin{prop}
In the lineage $L$ of a node $n$, the root is an ancestor of all nodes except itself, and $n$ is an ancestor of none.
\end{prop}

\begin{prop}
Let $L$ be a lineage of $T$, and $a, b, c$ three different nodes of $L$. Then
\end{prop}

\begin{prop}
Let $T$ be a rooted tree, and $a$ a node of $T$, and $d$ a descendant node of $a$. Then there exists exactly one ancestry path connecting $a$ and $d$.
\end{prop}
\begin{proof}
Existence by \ref{anc_path_existence}, unicity by \ref{anc_path_unicity}
\end{proof}
