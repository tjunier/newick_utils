...
Homo\footnote{To know the length of an edge, you can use \distance{} \code{-m
p -n} -- see \ref{sct:distance}.}) occupies five characters (up to rounding
error). And to use two columns per unit length, one would pass \code{-w -2},
etc.
...
\startalignment[center]
{\tfx
\starttabulate[p{0.4\textwidth}p{0.1\textwidth}p{0.4\textwidth}]
\typefile{display_21_txt.cmd} \typefile{display_21_txt.out}
\NC \NC
\typefile{display_22_txt.cmd} \typefile{display_22_txt.out} \NC\NR
\typefile{display_23_txt.cmd} \typefile{display_23_txt.out} \NC \NC
